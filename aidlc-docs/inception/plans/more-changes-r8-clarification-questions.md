# More Changes R8 — Clarification Questions (change request)

**Context**: You requested changes after R7 (right = output; left/top/bottom = input; out→out blocked).  
...
A) **Connection rules still wrong** (which sides can connect, green/red)

B) **Edge looks wrong after connect** (jumps, crosses node, ugly curve)

C) **Handles hard to see / hit / unclear which is output vs input**

D) **Layout / spacing** (Vertical / Horizontal / Layered)

E) **Node visuals** (avatar, Condition/Decision, library)
...
## Optional freeform
Describe the changes you want (bullets; what you see vs what you want):
